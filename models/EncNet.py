...
__all__ = ['EncNet', 'EncModule', 'get_encnet']


class EncNet(SegBaseModel):
    def __init__(self, nclass, backbone='resnet101', aux=False, se_loss=False, lateral=False,
...
def get_encnet(nclass=3, backbone='resnet101', pretrained=False, root='~/.torch/models',
               pretrained_base=True, **kwargs):
    
    model = EncNet(nclass=3, backbone=backbone, pretrained_base=pretrained_base, **kwargs)
